+++
title = "Putting a site on Netlify"
date = "2021-03-10"
author = "Sean"
cover = "img/putting-a-site-on-netlify/putting-a-site-on-netlify-cover.png"
coverAlt = "Netlify can host your Hugo site with CDN, continuous deployment, 1-click HTTPS, an admin GUI, and its own CLI"
description = "Putting a Hugo site up on Netlify for the first time"
+++
...
To get started I first had to create an account with Netlify. This is pretty straight forward as they allow you to just sign in and get started with your GitHub profile. Once I did that I was greeted with the following dashboard:

{{< image src="/img/putting-a-site-on-netlify/putting-a-site-on-netlify 1.png" alt="Netlify Dashboard" position="center" style="border-radius: 8px;" >}}
...
{{< image src="/img/putting-a-site-on-netlify/putting-a-site-on-netlify 2.png" alt="Create New Site Wizard for Netlify" position="center" style="border-radius: 8px;" >}}
...
Once you hit the deploy button Netlify will clone your selected repository and attempt to build it. At first it looked like I was going to be lucky and have everything just work as expected. Almost immediately though my build command failed as my selected theme was not compatible with the version of Hugo that Netlify was using and the below error was thrown:
...
For the next post I'll be taking a look at how Netlify handles serverless functions. I'll probably start by setting up a simple health check endpoint to get a hang of the process then see if we can cook up something a little more fun for the second half 😀
